...
#### If Antigravity (Google) — replaces deprecated Gemini CLI

```bash
oma agent:spawn debug "scan prompt with diagnosis context" {session_id} -w {workspace} -m antigravity
```
OMA invokes `agy --print --dangerously-skip-permissions` headlessly to capture the response.

#### If Gemini CLI (deprecated) or CLI Fallback
