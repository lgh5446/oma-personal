...
Antigravity replaces the deprecated Gemini CLI as of 2026-05 and reuses `.gemini/agents/{agent}.md` definitions for compatibility.
For headless dispatch, OMA invokes `agy --print --dangerously-skip-permissions` automatically.
For interactive use, open `antigravity chat` and run `/{agent-id}` slash commands with the oh-my-agent plugin.
